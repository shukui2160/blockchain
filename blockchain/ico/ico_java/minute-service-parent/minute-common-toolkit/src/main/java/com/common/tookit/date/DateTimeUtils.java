package com.common.tookit.date;
...
public class DateTimeUtils {
    ...
    public static boolean isLastAttendance(Date date) {
		...
		Date firstDayOfThisMonth = DateTimeUtils.getFirstDayOfMonth(currentTime);
	...
	public static Calendar getTodayFromHHMM(String time) {
		if (isBlank(time) || !time.matches("^(([01][0-9])|(2[0-3]))\\:([0-5][0-9])$")) {
			return null;
		}
	...
	public static boolean before (String earlyStr, String lateStr) {
		...
		Date late = null;
		try {
			early = DateTimeUtils.parseYYYYMMDD(earlyStr);
			late = DateTimeUtils.parseYYYYMMDD(lateStr);
		} catch (ParseException e) {
	 ...
	 * @return
	 */
	public static Date generateRandomDate(Date fromDate, Date toDate) throws Exception {
		long fromTime = fromDate.getTime();
		long toTime = toDate.getTime();
		
		if (toTime <= fromTime) {
			throw new Exception("fromDate can't equal/large than toDate");
		}
		
		long timeDifference = toTime - fromTime;
		float randomNum = new Random().nextFloat();
		
		long ramdomTime = (long) (timeDifference * randomNum);
		
		return new Date(ramdomTime);
	}
